...
import { Search, Filter, Star, MapPin as MapPinIcon, ChevronRight, Loader2, Heart, MessageSquare, ArrowRight, BookOpen, Users } from 'lucide-react';
import { Link } from 'react-router-dom';
import { motion, AnimatePresence } from 'motion/react';
import { Kindergarten, SchoolType } from '../data';
...
export default function Home() {
  const [schools, setSchools] = useState<Kindergarten[]>([]);
  ...
  const filteredSchools = useMemo(() => {
    return schools.filter(school => {
      ...
      const matchesPhilosophy = activePhilosophy === '全部' || (school.philosophyTags && school.philosophyTags.includes(activePhilosophy));
      return matchesSearch && matchesType && matchesDistrict && matchesPhilosophy;
    });
  }, [schools, searchQuery, activeType, activeDistrict, activePhilosophy]);

  const mapCenter: [number, number] = [22.5333, 113.9305]; // Shenzhen Nanshan Center as initial
  ...
  return (
    <div className="flex flex-col bg-warm-white min-h-screen">
      ...
      {/* Banner 2: Map Yellow Pages Area */}
        ...
        <div className="bg-white rounded-[40px] shadow-2xl flex flex-col lg:flex-row h-[700px] overflow-hidden border border-trust-blue/5">
          ...
          <div className="w-full lg:w-[420px] flex flex-col bg-white border-r border-trust-blue/5">
            ...
            <div className="flex-grow overflow-y-auto px-6 pb-6 space-y-4 no-scrollbar">
               {loadingSchools ? (
                 <div className="flex flex-col items-center justify-center py-20 text-trust-blue/20">
                   <Loader2 className="animate-spin mb-4" size={32} />
                   <p className="text-xs font-bold">同步最新园所资料...</p>
                 </div>
               ) : (
                        ...
                        <div className="flex flex-col justify-center flex-grow min-w-0">
                           <div className="flex items-center gap-2 mb-1">
                              <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${
                                school.type === '公立' ? 'bg-green-100 text-green-700' :
                                school.type === '国际' ? 'bg-purple-100 text-purple-700' :
                           ...
                           <h3 className="font-heading font-black text-trust-blue text-base leading-tight truncate pr-8">{school.name}</h3>
      ...
      {/* Banner 3: Kindergarten Notes Square */}
      <section id="notes-section" className="max-w-7xl mx-auto w-full px-4 mb-32">
        ...
        {loadingNotes ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="aspect-[3/4] bg-white rounded-[32px] animate-pulse"></div>
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {notes.map((note) => (
              <Link to={`/school/${note.schoolId}`} key={note.id} className="block">
                <motion.div 
                  whileHover={{ y: -8 }}
                  className="bg-white rounded-[32px] overflow-hidden shadow-sm hover:shadow-xl border border-trust-blue/5 group h-full"
                >
      ...
      {/* Banner 4: Bottom Call-to-Action */}
